...
// HashRouter: 表示一个路由的根容器，将来，所有的路由相关的东西，都要包裹在 HashRouter里面，而且，一个网站中，只需要使用一次 HashRouter就好了；
  ...
  render() {
    ...
    return <HashRouter>
      <div>
        ...
        <Link to="/movie/top10/10">电影</Link>
        <Link to="/about">关于</Link>


        {/* 
        Route创建的标签，就是路由规则， 其中path 表示要匹配的路由，component 表示要展示的组件, exact 表示启动精确匹配模式； 
        Route 具有两种身份：1.是一个路由匹配规则；2.是一个占位符，表示将来匹配到的组件都放到这个位置;
      ...
      */}
        ...
        <hr/>
        {/* 注意：
          1. 默认情况下，路由中的规则，是模糊匹配的，如果路由可以部分匹配成功，就会展示这个路由对应的组件;
          eg: /movie/top10/10 可以匹配到 /movie
